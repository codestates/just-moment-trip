  ...
  get: async (req, res) => {
    try {
      ...
      const requestKakaoToken = await axios.post(
        `${baseUrl}?grant_type=authorization_code&client_secret=${process.env.KAKAO_SECRET}&client_id=${process.env.KAKAO_CLIENT}&redirect_uri=http://localhost:9000/oauth/callback/kakao&code=${code}`,
        {
          headers: {
